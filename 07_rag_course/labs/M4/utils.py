...
def generate_with_multiple_input2(messages: List[Dict], top_p: float = 1, temperature: float = 1, max_tokens: int = 500,
...
def call_llm_with_context(prompt: str, context: list, role: str = 'user', **kwargs):
    ...
    """
    context.append({'role': role, 'content': prompt})
    response = generate_with_multiple_input2(context, **kwargs)
    context.append(response)
    return response
